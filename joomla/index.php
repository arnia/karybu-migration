...
if($path) {
    ...
    if(!$db_info) {
        $errMsg = "Could not retrieve Joomla database config file.";
    } else {
        $oMigration->setDBInfo($db_info);
        $oMigration->setCharset('UTF-8', 'UTF-8');
        if($message = $oMigration->dbConnect()) {
            $errMsg = $message;
        }
        else {
            $step = 2;
...
if($step == 2) {
    // Count articles
    $query = "select count(*) as count from {$db_info->db_table_prefix}_content";
    ...
    if ($total_member_count>0) {
        $member_division_cnt = (int)(($total_member_count-1)/$member_division) + 1;
    }
...
<title>Joomla 2.5 Data Export Tool</title>
<style type="text/css">
...
<body>
    <h1>Joomla 2.5 Data Export Tool</h1>
    <?php
        if($errMsg) {
    ?>
    <hr />
    ...
    <form action="./index.php" method="post">
        <h3>Step 1. Enter Joomla project path</h3>

        <ul>
            <li>
                Please enter directory path. Here are some examples:

                <blockquote>
                1) /home/user/public_html/joomla25<br />
                2) ../joomla25
                </blockquote>
